...
@JsonIgnoreProperties({"title","middleName","dob","email","telephone","mobile","jobTitle",
	"staffNumber","address","postcode","city","licenseNo","licenseValidFrom",
	"licenseValidTo","endDate","active","webAccess","catId"})
@XmlRootElement(name = "DriverName")
public class DriverNameDto {
	...
	public static List<DriverNameDto> transform(List<Driver> entityList) {
		
		List<DriverNameDto> listOfDriversName = new ArrayList<DriverNameDto>();
		
		
		for (Driver entity : entityList) {
			
			DriverNameDto dto = new DriverNameDto();
			...
			listOfDriversName.add(dto);
		}
		return listOfDriversName;
	}
